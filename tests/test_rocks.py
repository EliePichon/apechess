...
def test_bestmove_with_rocks(fen, description):
    ...
    payload = {"fen": fen, "maxdepth": 6, "top_n": 3}
...
def main():
    ...
    results.append(
        test_rock_blocks_sliding_piece(
    ...
    results.append(
        test_knight_jumps_over_rock(fen="8/8/8/8/3O4/8/3N4/8 w - - 0 1", description="Knight at d2 can jump over rock at d4")
    )
    ...
    results.append(
        test_bestmove_with_rocks(
            fen="8/8/8/8/3O4/2n5/3N4/8 w - - 0 1", description="Calculate bestmove in position with rock at d4"
        )
    )
