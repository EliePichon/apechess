...
def test_fast_path():
    ...
    check(
        "bestmove is 4-5 chars",
        len(result["bestmove"]) in (4, 5),
        f"got '{result['bestmove']}' ({len(result['bestmove'])} chars)",
    )
    check("has scored_moves", "scored_moves" in result)
    check("scored_moves has 1 entry", len(result["scored_moves"]) == 1, f"got {len(result['scored_moves'])}")
    check(
        "scored_moves entry is (str, int)",
    ...
    check("clutchness is None for top_n=1", result.get("clutchness") is None, f"got {result.get('clutchness')}")
...
def test_standard_path():
    ...
    check("returns dict", isinstance(result, dict))
    check("scored_moves has 5 entries", len(result["scored_moves"]) == 5, f"got {len(result['scored_moves'])}")

    # Check all entries are (str, int)
    all_valid = all(isinstance(m, str) and isinstance(s, (int, float)) for m, s in result["scored_moves"])
    check("all scored_moves are (str, number)", all_valid)

    # Check scores are descending
    scores = [s for _, s in result["scored_moves"]]
    check("scores are descending", scores == sorted(scores, reverse=True), f"got {scores}")

    check("clutchness is int", isinstance(result.get("clutchness"), int), f"got {type(result.get('clutchness'))}")
...
def test_ignore_squares():
    ...
    for move_str, _ in result["scored_moves"]:
        from_sq = move_str[:2]
        check(f"move {move_str} not from ignored square", from_sq not in ("g1", "b1"), f"from {from_sq}")

    # bestmove should also not start from ignored squares
    best = result["bestmove"]
    check(f"bestmove {best} not from ignored square", best[:2] not in ("g1", "b1"), f"from {best[:2]}")
...
def test_no_legal_moves():
    ...
    check("bestmove is (none)", result["bestmove"] == "(none)", f"got '{result['bestmove']}'")
    check("scored_moves is empty", result["scored_moves"] == [], f"got {result['scored_moves']}")
...
def test_clutchness():
    ...
    check("clutchness is not None for top_n=3", result.get("clutchness") is not None, f"got {result.get('clutchness')}")
    check("clutchness is int", isinstance(result.get("clutchness"), int), f"got type {type(result.get('clutchness'))}")
    check("clutchness >= 0", result.get("clutchness", -1) >= 0, f"got {result.get('clutchness')}")
    ...
    if len(result["scored_moves"]) >= 2:
        ...
        check("clutchness matches score gap", result["clutchness"] == gap, f"clutchness={result['clutchness']}, gap={gap}")
...
def test_black_to_move():
    ...
    from_sq = best[:2]
    from_rank = from_sq[1]
    check(f"black move {best} from rank 5-8", from_rank in "5678", f"from rank {from_rank}")
...
def test_return_contract():
    ...
    check("exact keys match", set(result.keys()) == expected_keys, f"got {set(result.keys())}")
