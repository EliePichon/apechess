...
def test_clutchness_on_bestmove():
    ...
    t.test("clutchness is numeric", isinstance(data.get("clutchness"), (int, float)), f"got {type(data.get('clutchness'))}")
...
def test_clutchness_not_present_without_flag():
    ...
    data = r.json()
    t.test("no clutchness field without flag", "clutchness" not in data, f"keys: {list(data.keys())}")
...
def test_session_stats():
    ...
    t.test("tp_move_size > 0 after search", data.get("tp_move_size", 0) > 0, f"got {data.get('tp_move_size')}")
...
def test_session_isolation():
    ...
    t.test("session 1 position advanced (e2e4 now illegal)", r.status_code == 400, f"got {r.status_code}")
